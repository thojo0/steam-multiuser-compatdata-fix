...
* **Make Mount Failures Obvious:** To be alerted if the bind mount fails (preventing Steam from writing to the *shared* folder by mistake), you can make the *original* shared `compatdata` directories unwritable. Run this on the *actual* shared library `compatdata` folders (when they are not mounted over):
    ```bash
    # Example for a shared library at /shared/steamlib
    sudo chown root:root /shared/steamlib/steamapps/compatdata
